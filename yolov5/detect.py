                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if names[c] == 'person' and acc > 0.5:
                            ...
                            width = xyxy_num[2] - xyxy_num[0]
                            height = xyxy_num[3] - xyxy_num[1]
                            xy = [0, 0, 0, 0]
                            if width > height:
                                delta = (width - height) / 2
                                xy[1] = 0 if xyxy_num[1] < delta else xyxy_num[1] - delta
                                xy[3] = xyxy_num[3] + delta + (delta - xyxy_num[1] if xyxy_num[1] < delta else 0)
                                xy[0], xy[2] = xyxy_num[0], xyxy_num[2]
                            else:
                                delta = (height - width) / 2
                                xy[0] = 0 if xyxy_num[0] < delta else xyxy_num[0] - delta
                                xy[2] = xyxy_num[2] + delta + (delta - xyxy_num[0] if xyxy_num[0] < delta else 0)
                                xy[1], xy[3] = xyxy_num[1], xyxy_num[3]
                            xyxy = [torch.tensor(np.array(i)) for i in xy]
                            # save crop
                            # image = Image.open(path)
                            # cropped = image.crop((xyxy_num[0], xyxy_num[1], xyxy_num[2], xyxy_num[3]))
